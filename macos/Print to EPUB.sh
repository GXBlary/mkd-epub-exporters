#!/bin/bash
# macOS PDF Service: Print to EPUB
# Receives the generated PDF path as the first argument ($1)

PDF_PATH="$1"

# 1. Ask the user for the output path using native AppleScript
OUT_PATH=$(osascript -e 'tell application "System Events" to POSIX path of (choose file name with prompt "Save EPUB file as:" default name "document.epub")' 2>/dev/null)
...
/usr/local/bin/markitdown-cli -out "$TEMP_DIR" -format epub "$PDF_PATH"

# 5. Move the converted file to the user's selected path
CONV_FILE=$(find "$TEMP_DIR" -type f \( -name "*.epub" \) | head -n 1)
if [ -f "$CONV_FILE" ]; then
